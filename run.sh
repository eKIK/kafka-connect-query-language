...
travis_fold start git.checkout
  if [[ ! -d eKIK/kafka-connect-query-language/.git ]]; then
    ...
    travis_cmd git\ -C\ eKIK/kafka-connect-query-language\ fetch\ origin --assert --echo --retry --timing
    travis_cmd git\ -C\ eKIK/kafka-connect-query-language\ reset\ --hard --assert --echo
  fi
  rm -f $HOME/.netrc
  travis_cmd cd\ eKIK/kafka-connect-query-language --echo
...
export TRAVIS_BUILD_DIR=$HOME/build/eKIK/kafka-connect-query-language
...
export TRAVIS_REPO_SLUG=eKIK/kafka-connect-query-language
...
travis_fold start before_install
  travis_cmd chmod\ \+x\ gradlew --assert --echo --timing
...
travis_fold start install
  travis_cmd ./gradlew\ check --assert --echo --timing
